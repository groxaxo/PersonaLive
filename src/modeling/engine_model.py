...
class EngineModel:
        ...
        try:
            ...
            def get_safe_shape(engine, name):
                ...
                if -1 in shape:
                    ...
                    if profile:
                        print(
                            f"[EngineModel] Detected dynamic shape for {name}: {shape} -> Using Max Profile: {profile[2]}"
                        )
                        return profile[2]
                    else:
                        ...
                        # 这里为了防止报错，可以尝试打印警告
                        print(
                            f"[EngineModel] Warning: Dynamic output {name} has no profile. Mem alloc might fail."
                        )
                return shape
    ...
    def __call__(
        self,
        skip_check=SKIP_ENGINE_MODEL_CHECK,
        output_list=[],
        return_tensor=False,
        synchronize=True,
        **inputs,
        ...
        if not skip_check:
            for name in inputs:
                ...
                assert match_dtype(
                    inputs[name].dtype, trt.nptype(self.input_dtypes[name])
                )
        if not (self.extra_lock is None):
            self.extra_lock.acquire()
        self.ctx.push()
        ...
        try:
            ...
            if return_tensor:
                for name in output_list:
                    ...
                    ptr = t.data_ptr()
                    cuda.memcpy_dtod_async(
                        ptr, self.doutputs[name], self.output_nbytes[name], self.stream
                    )
    ...
    def prefill(self, skip_check=SKIP_ENGINE_MODEL_CHECK, synchronize=True, **inputs):
        if not (skip_check):
            for name in inputs:
                ...
                assert match_shape(
                    inputs[name].shape,
                    self.input_shapes[name] if in_input else self.output_shapes[name],
                )
                assert match_dtype(
                    inputs[name].dtype,
                    trt.nptype(
    ...
    def link(self, other, var_map, skip_check=SKIP_ENGINE_MODEL_CHECK):
        ...
        if not (skip_check):
            for source in var_map:
                ...
                assert match_shape(
                    other.output_shapes[source], self.input_shapes[target]
                )
                assert match_dtype(
                    other.output_dtypes[source], self.input_dtypes[target]
                )
    ...
    def bind(self, var_map, skip_check=SKIP_ENGINE_MODEL_CHECK):
        if not (skip_check):
            for source in var_map:
                ...
                assert target in self.input_names
                assert match_shape(
                    self.output_shapes[source], self.input_shapes[target]
                )
                assert match_dtype(
                    self.output_dtypes[source], self.input_dtypes[target]
                )
