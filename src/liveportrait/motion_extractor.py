...
class MotionExtractor(ModelMixin):
    ...
    def get_kp(self, kp_info):
        ...
        angles_raw = torch.cat(
            [kp_info["pitch"], kp_info["yaw"], kp_info["roll"]], dim=0
        )  # (3, 66)
    ...
    def interpolate_kps(self, ref, motion, num_interp, t_scale=0.5, s_scale=0):
        ...
        angles_raw = torch.cat(
            [kp1["pitch"], kp1["yaw"], kp1["roll"]], dim=0
        )  # (3, 66)
        angles_deg = self.headpose_pred_to_degree(angles_raw)  # (B, 3)
        pitch_1, yaw_1, roll_1 = torch.chunk(angles_deg, chunks=3, dim=0)

        angles_raw = torch.cat(
            [kp2["pitch"], kp2["yaw"], kp2["roll"]], dim=0
        )  # (3, 66)
        angles_deg = self.headpose_pred_to_degree(angles_raw)  # (B, 3)
        pitch_2, yaw_2, roll_2 = torch.chunk(angles_deg, chunks=3, dim=0)

        pitch_interp = self.interpolate_tensors(
            pitch_1, pitch_2[:1], num_interp
        )  # Bx(num_interp)x1
        yaw_interp = self.interpolate_tensors(
            yaw_1, yaw_2[:1], num_interp
        )  # Bx(num_interp)x1
        roll_interp = self.interpolate_tensors(
            roll_1, roll_2[:1], num_interp
        )  # Bx(num_interp)x1
    ...
    def interpolate_kps_online(self, ref, motion, num_interp, t_scale=0.5, s_scale=0):
        kp_all = self.detector(torch.cat([ref, motion], dim=0).to(self.dtype))
        kp1 = {k: v[:1] for k, v in kp_all.items()}
        kp2 = {k: v[1:] for k, v in kp_all.items()}
        kp_frame1 = {k: v[:1] for k, v in kp2.items()}
