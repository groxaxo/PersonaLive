...
class PersonaLive:
    def __init__(self, args, device=None):
        ...
        self.unet_work = EngineModel(
            engine_file_path=cfg.tensorrt_target_model, device_int=self.device.index
        )
        self.unet_work.bind(
            {
        ...
        self.timesteps = timesteps.repeat_interleave(
            cfg.temporal_window_size, dim=0
        ).long()
        ...
        self.cfg = cfg
        self.reset()
        self.reference_hidden_states_names = [
            "d00",
            "d01",
            "d10",
            "d11",
    ...
    def fuse_reference(self, ref_image):  # pil input
        ...
        self.unet_work.prefill(
            encoder_hidden_states=encoder_hidden_states, synchronize=False
        )
        ...
        self.unet_work.prefill(
            **{
                name: self.reference_hidden_states[name]
        ...
        self.ref_image_latents = ref_image_latents.unsqueeze(2).repeat(
        ...
        self.noisy_latents_first = self.scheduler.add_noise(
            init_latents, noise, self.timesteps[:padding_num]
        )
    ...
    def process_input(self, images):
        ...
        if self.first_frame:
            ...
            motion_hidden_states = self.interpolate_tensors(
        ...
        if len(idx_to_add) > 0 and self.num_khf < 3:
            ...
            for name in self.reference_hidden_states_names:
                self.reference_hidden_states[name] = torch.cat(
                    [self.reference_hidden_states[name], reference_hidden_states[name]],
                    dim=1,
                )
